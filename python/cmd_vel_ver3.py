...
class btr_2024(Node):
    def __init__(self, topicName):
        ...
        self.sub3 = self.create_subscription(Point, self.topicName + "/btr/centerPoint", , qos)
        self.sub4 = self.create_subscription(Point, self.topicName + "/btr/leftPoint", lambda msg: setattr(self, 'leftPoint', msg), qos)
        self.sub5 = self.create_subscription(Point, self.topicName + "/btr/rightPoint", lambda msg: setattr(self, 'rightPoint', msg), qos)
        self.sub6 = self.create_subscription(Point, self.topicName + "/btr/forwardPoint", lambda msg: setattr(self, 'forwardPoint', msg), qos)
    ...
    def w_waitOdometry(self):
        # ROS 2にはheader.seqがないため、タイムスタンプの変更で同期を待ちます
    ...
    def w_turnVelocity(self, theta, ori, quick = False):
        ...
        b = 5
        return float(velocity * velocitySign / b)
...
if __name__ == '__main__':
  ...
  while rclpy.ok() and not rclpy.is_shutdown():
